...
miles[grep('99',milestone),x0:=x0-40]
miles[milestone=='Ban on public gatherings in Danang',x0:=x0-25]
miles[grep('Community search',milestone),x0:=x0+30]
miles[grep('Danang citywide',milestone),x0:=x0-20]
...
miles[grep('Community search',milestone),x1:=x1-28]
miles[grep('Danang citywide',milestone),x1:=x1+15]

############# Y coordinate, generating "ladder"

miles[,y0:=rep(seq(52,22,-10),1+.N/4)[1:.N],by='Region']
